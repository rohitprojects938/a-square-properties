...
// ============================================================
// LocationManager — One-time permission, reverse geocoding,
// localStorage caching, DB sync, manual update trigger
// ============================================================
window.LocationManager = (function() {
  const LS_PERMISSION = 'location_permission';  // 'granted' | 'denied' | null
  const LS_LOCATION   = 'user_location';         // JSON: {lat,lng,city,state,area,pincode,timestamp}
  const GEOCODE_TTL   = 60 * 60 * 1000;         // 1 hour between auto re-geocodes

  // --- Internal helpers ---

  function getPermission() {
    return localStorage.getItem(LS_PERMISSION); // 'granted' | 'denied' | null
  }
  ...
  function saveToLocalStorage(permission, locData) {
    localStorage.setItem(LS_PERMISSION, permission);
    if (locData) {
      localStorage.setItem(LS_LOCATION, JSON.stringify({ ...locData, timestamp: Date.now() }));
    }
  }

  // Fire a custom event so any listener (Popular Near You, etc.) can react
  function emitLocationChanged(locData) {
    const ev = new CustomEvent('locationChanged', { detail: locData });
    window.dispatchEvent(ev);
  }

  // Nominatim reverse geocoding — free, no key required
  // Returns {city, state, area, pincode} or {}
  async function reverseGeocode(lat, lng) {
    try {
      const url = `https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=${lat}&lon=${lng}&accept-language=en`;
      const res = await fetch(url, {
        headers: { 'Accept-Language': 'en', 'User-Agent': 'HouseRenter/1.0' }
      });
      ...
      return {
        ...
        area:    addr.suburb || addr.neighbourhood || addr.quarter || addr.locality || '',
        pincode: addr.postcode || ''
      };
    } catch (err) {
      console.warn('⚠️ Reverse geocode failed:', err.message);
      return {};
    }
  }

  // Persist to DB for logged-in users (fire-and-forget, never blocks UX)
  async function syncToDb(permission, locData) {
    try {
      const token = localStorage.getItem('token');
      if (!token) return; // Anonymous user — localStorage only
      const body = permission === 'denied'
        ? { permission: 'denied' }
        : {
            permission: 'granted',
            lat:     locData.lat,
      ...
      console.warn('⚠️ Location DB sync failed (non-blocking):', err.message);
    }
  }

  // Core GPS fetch + geocode + save flow
  async function fetchGpsAndSave(onSuccess, onError) {
    ...
    // Try option 1: enableHighAccuracy: false, low timeout first (fast cell-tower/Wi-Fi check)
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        await processGpsCoords(position.coords.latitude, position.coords.longitude, onSuccess, onError);
      },
      (error) => {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (error2) => {
            console.warn('⚠️ Geolocation option 2 failed:', error2.message);
            // Save the denial only if user explicitly blocked permission
            if (error2.code === error2.PERMISSION_DENIED) {
              saveToLocalStorage('denied', null);
              localStorage.setItem('location_denied_at', Date.now().toString());
              syncToDb('denied', null);
            }
            if (onError) onError(error2.message);
          },
  ...
  async function processGpsCoords(lat, lng, onSuccess, onError) {
    try {
      // Reverse geocode to get readable address
      const geoData = await reverseGeocode(lat, lng);
      const locData = { lat, lng, ...geoData };

      saveToLocalStorage('granted', locData);
      localStorage.removeItem('location_denied_at');
      syncToDb('granted', locData); // non-blocking
      emitLocationChanged(locData);

      if (onSuccess) onSuccess(locData);
    } catch (err) {
      console.warn('⚠️ Process coords failed:', err.message);
  ...
  return {
    /** Returns stored location object or null */
    getStoredLocation,
    ...
    /**
     * Called on page load. If already granted, emits locationChanged.
     */
    init() {
      ...
      if (perm === 'granted' && loc) {
        emitLocationChanged(loc);
      }
    },

    /**
     * Requests location permission popup.
     * Implements Smart Re-Prompt Logic (3-Day Reminder):
     * - Case 1: If granted and loc exists, stop forever.
     * - Case 2 & 3: If denied, wait 3 days since location_denied_at.
     * - Case 4: Stop forever after successful location.
     */
    requestOnce() {
      const perm = getPermission();
      const loc = getStoredLocation();
      const deniedAt = localStorage.getItem('location_denied_at');

      // Stop forever if location was ever successfully obtained
      if (loc && loc.lat && loc.lng) {
        return;
      }
      if (perm === 'granted' && loc && loc.lat && loc.lng) {
        return;
      }
      ...
      if (perm === 'denied') {
        ...
        const diffDays = (Date.now() - parseInt(deniedAt)) / (1000 * 60 * 60 * 24);
        if (diffDays < 3) {
          return; // Within 3 days cooldown
        }
      }

      // Show popup after a short delay so the page loads first
      setTimeout(() => {
        // Double-check one last time in case they updated it manually
        const currentPerm = getPermission();
        const currentLoc = getStoredLocation();
        ...
        const overlay = document.getElementById('location-sheet-overlay');
        if (sheet && overlay) {
    ...
    async handleAllow(onSuccess, onError) {
      await fetchGpsAndSave(onSuccess, onError);
    },

    /** Called when user clicks "Maybe Later" / X / overlay */
    handleDeny() {
      saveToLocalStorage('denied', null);
      localStorage.setItem('location_denied_at', Date.now().toString());
      syncToDb('denied', null); // non-blocking
    },

    /**
     * Manual update trigger.
     * Always fetches fresh GPS (ignores cache), updates localStorage + DB.
     */
    async updateManually(onSuccess, onError) {
      await fetchGpsAndSave(onSuccess, onError);
    },

    /**
     * Save fallback manual selection of State/City/Area.
     */
    saveManualLocation(state, city, area, coords) {
      const locData = {
        lat: coords.lat,
        lng: coords.lng,
        country: 'India',
      ...
      // Mark as granted & clear re-prompt timestamp
      saveToLocalStorage('granted', locData);
      localStorage.removeItem('location_denied_at');
      syncToDb('granted', locData);
      emitLocationChanged(locData);
      return locData;
    },

    /**
     * Auto-geocodes a text address via Nominatim.
     * Used by post.html to fill lat/lng from address+city+pincode.
     */
    async geocodeAddress(addressStr) {
      if (!addressStr || addressStr.trim().length < 3) return null;
      try {
        const encoded = encodeURIComponent(addressStr.trim());
        const url = `https://nominatim.openstreetmap.org/search?format=jsonv2&q=${encoded}&limit=1&countrycodes=in`;
        const res = await fetch(url, {
          headers: { 'Accept-Language': 'en', 'User-Agent': 'HouseRenter/1.0' }
        });
        ...
        console.warn('⚠️ Forward geocode failed:', err.message);
        return null;
      }
    }
  };
