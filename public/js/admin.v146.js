...
let servicePage = 1;
const serviceLimit = 15;

async function loadServicesManager() {
  const search = document.getElementById('adm-service-search').value.trim();
  const status = document.getElementById('adm-service-filter-status').value;
  const category = document.getElementById('adm-service-filter-category').value;

  let url = `/api/admin/services?page=${servicePage}&limit=${serviceLimit}`;
  ...
  if (res.success) {
    ...
    const tbody = document.getElementById('services-manager-table-body');
    if (!tbody) return;

    if (servicesList.length === 0) {
      tbody.innerHTML = `<tr><td colspan="10" style="text-align:center; padding:24px; color:var(--adm-muted);">No services found.</td></tr>`;
      document.getElementById('adm-service-pagination-text').textContent = 'Showing 0 of 0 services';
      return;
    }

    tbody.innerHTML = servicesList.map(s => {
      let statusColor = 'badge-yellow';
      if (s.status === 'approved') statusColor = 'badge-green';
      if (s.status === 'rejected') statusColor = 'badge-red';
      if (s.status === 'suspended') statusColor = 'badge-gray';

      const dateStr = s.created_at ? new Date(s.created_at).toLocaleDateString() : 'N/A';

      // Status action buttons dynamically showing up depending on current status
      let approveBtn = '';
      let rejectBtn = '';
      let suspendBtn = '';
      let restoreBtn = '';
      ...
      if (s.status === 'pending') {
        approveBtn = `<button class="adm-btn adm-btn-ghost adm-btn-sm" onclick="moderateServiceStatus(${s.id}, 'approved')" title="Approve" style="color:var(--adm-green);"><i data-lucide="check"></i></button>`;
        rejectBtn = `<button class="adm-btn adm-btn-ghost adm-btn-sm" onclick="moderateServiceStatus(${s.id}, 'rejected')" title="Reject" style="color:var(--adm-red);"><i data-lucide="x"></i></button>`;
      } else if (s.status === 'approved') {
        suspendBtn = `<button class="adm-btn adm-btn-ghost adm-btn-sm" onclick="moderateServiceStatus(${s.id}, 'suspended')" title="Suspend" style="color:var(--adm-yellow);"><i data-lucide="shield-alert"></i></button>`;
      } else if (s.status === 'suspended' || s.status === 'rejected') {
        restoreBtn = `<button class="adm-btn adm-btn-ghost adm-btn-sm" onclick="moderateServiceStatus(${s.id}, 'approved')" title="Restore / Approve" style="color:var(--adm-green);"><i data-lucide="rotate-ccw"></i></button>`;
      }
      ...
      return `
        <tr>
          ...
          <td><span class="adm-badge ${statusColor}">${s.status.toUpperCase()}</span></td>
          <td>${dateStr}</td>
          <td>
    ...
    const start = (servicePage - 1) * serviceLimit + 1;
    const end = Math.min(servicePage * serviceLimit, total);
    document.getElementById('adm-service-pagination-text').textContent = total > 0 ? `Showing ${start}-${end} of ${total} services` : 'Showing 0 of 0 services';

    document.getElementById('adm-service-prev-btn').disabled = servicePage === 1;
    document.getElementById('adm-service-next-btn').disabled = end >= total;

    if (window.lucide) window.lucide.createIcons();
  }
}

window.prevServicePage = () => {
  if (servicePage > 1) {
    servicePage--;
    loadServicesManager();
  }
...
window.nextServicePage = () => {
  servicePage++;
  loadServicesManager();
};

window.moderateServiceStatus = async (sid, newStatus) => {
  if (confirm(`Change service status to ${newStatus}?`)) {
    const res = await apiRequest(`/api/admin/services/${sid}/status`, 'PUT', { status: newStatus });
    if (res.success) {
...
window.editServiceModal = (sid) => {
  ...
  document.getElementById('service-provider').value = s.provider_name || '';
  document.getElementById('service-category').value = s.category || 'painter';
  document.getElementById('service-mobile').value = s.mobile_number || '';
  document.getElementById('service-whatsapp').value = s.whatsapp_number || '';
  document.getElementById('service-city').value = s.city || '';
  document.getElementById('service-address').value = s.address || '';
  document.getElementById('service-experience').value = s.experience || '';
  document.getElementById('service-price').value = s.starting_price || '';
  document.getElementById('service-image').value = s.image_url || '';
  document.getElementById('service-days').value = s.available_days || '';
...
window.saveAdminService = async (e) => {
  ...
  const payload = {
    ...
    provider_name: document.getElementById('service-provider').value,
    category: document.getElementById('service-category').value,
    mobile_number: document.getElementById('service-mobile').value,
    whatsapp_number: document.getElementById('service-whatsapp').value || document.getElementById('service-mobile').value,
    city: document.getElementById('service-city').value,
    address: document.getElementById('service-address').value,
    experience: document.getElementById('service-experience').value,
    starting_price: document.getElementById('service-price').value ? parseFloat(document.getElementById('service-price').value) : null,
...
window.deleteService = async (sid) => {
  if (confirm('Delete this service listing completely?')) {
