...
window.LocationManager = (function() {
  ...
  async function fetchGpsAndSave(onSuccess, onError) {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (error) => {
        ...
        // If denied by browser, save the denial
  ...
  return {
    ...
    /**
     * Called on page load. If no prior decision exists, shows the
     * location bottom-sheet popup (one time only).
     * If already granted AND cached location is fresh, emits locationChanged
     * so pages can render immediately without waiting for a new GPS call.
     */
    init() {
      ...
      if (perm === 'granted' && loc) {
        // Emit immediately so Popular Near You loads at once
        emitLocationChanged(loc);
      }
      // Otherwise the popup or fallback handles it
    },

    /**
     * Requests location permission ONLY IF user has never responded.
     * Shows the #location-sheet bottom sheet.
     * Call this once per first visit.
     */
    requestOnce() {
      const perm = getPermission();
      if (perm !== null) return; // Already decided — never show again automatically

      // Show popup after a short delay so the page loads first
      setTimeout(() => {
        // Double-check: another tab may have stored a decision in the meantime
        if (localStorage.getItem(LS_PERMISSION) !== null) return;
    ...
    /**
     * Manual "📍 Update My Location" trigger.
     * Always fetches fresh GPS (ignores cache), updates localStorage + DB,
     * and fires locationChanged event for all listeners.
     * @param {Function} onSuccess  called with locData on success
